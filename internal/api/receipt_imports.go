...
func (r *ReceiptImport) run(newFetcher fetcherFactory, q *db.Queries) {
	...
	if receiptID > 0 {
		...
		dbReceipt, err := q.GetReceiptByAccessKey(ctx, parsed.Receipt.AccessKey)
		if err == nil {
			r.ReceiptID = &dbReceipt.ID
		} else {
			slog.Warn("failed to fetch receipt ID for idempotent duplicate", "access_key", parsed.Receipt.AccessKey, "err", err)
		}
	}
	r.UpdatedAt = time.Now()
	r.mu.Unlock()
