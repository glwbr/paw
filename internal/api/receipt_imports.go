...
func (r *ReceiptImport) MarshalJSON() ([]byte, error) {
	...
	if r.status == StatusWaitingCaptcha {
		captchaURL = fmt.Sprintf("/receipts/imports/%s/captcha", r.ID)
	}
...
func (s *Server) createImport(w http.ResponseWriter, r *http.Request) error {
	...
	if req.AccessKey != "" && req.QRURL != "" {
		return apierrors.BadRequest("access_key and qr_url are mutually exclusive", nil)
	}
	if req.AccessKey == "" && req.QRURL == "" {
		return apierrors.BadRequest("either access_key or qr_url is required", nil)
	}

	if req.AccessKey != "" && !accessKeyPattern.MatchString(req.AccessKey) {
		return apierrors.BadRequest("invalid access_key: must be 44 numeric digits", nil)
	}
